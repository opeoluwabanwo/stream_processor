...
run-app-with-pubsub:
	...
	docker exec --detach stream-processor-cont bash -c 'cd /app/infrastructure/scripts && source config.sh && cd /app && python stream_processor/beam/stream_processor.py --runner DirectRunner --streaming_engine pubsub'
